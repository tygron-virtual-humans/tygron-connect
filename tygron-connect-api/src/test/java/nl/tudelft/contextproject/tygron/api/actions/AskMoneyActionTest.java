...
public class AskMoneyActionTest {
  AskMoneyAction action;
  ...
  public void testActionWithMoney() {
    action = new AskMoneyAction(environmentWithMoney);
    action.askMoney(1, 10d);
    Mockito.verify(connection).execute(eq("event/PlayerEventType/MONEY_TRANSFER_ASK/"), eq(CallType.POST),
            any(StringResultHandler.class), eq(true), any(AskMoneyAction.AskMoneyRequest.class));
  }

  @Test
  public void testActionNoMoney() {
    action = new AskMoneyAction(environmentNoMoney);
    action.askMoney(1, 10d);
