...
public class ZoneLoaderTest {
  ...
  public void setup() {
    ...
    Mockito.when(connection.execute(Mockito.anyString(), Mockito.eq(CallType.GET), 
        Mockito.any(ZoneListResultHandler.class), Mockito.eq(true))).thenReturn(new ZoneList(result));
    
    loader = new ZoneListLoader();
  }
  
  @Test
  public void loadTest() {
    assertEquals(4, loader.load().size());
  }
  
  @Test
  public void getTest() {
    assertEquals(4, loader.get().size());
  }
  
  @Test
  public void getDataClassTest() {
    assertEquals(ZoneList.class, loader.getDataClass());
  }
