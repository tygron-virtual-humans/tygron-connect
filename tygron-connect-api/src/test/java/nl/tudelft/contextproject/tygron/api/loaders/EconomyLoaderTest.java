...
public class EconomyLoaderTest {
  EconomyListLoader loader;
  ...
  public void setup() {
    ...
    Mockito.when(connection.execute(Mockito.anyString(), Mockito.eq(CallType.GET), 
        Mockito.any(EconomyListResultHandler.class), Mockito.eq(true))).thenReturn(new EconomyList(result));
    
    loader = new EconomyListLoader();
  }
  ...
  public void getDataClassTest() {
    assertEquals(EconomyList.class, loader.getDataClass());
  }
